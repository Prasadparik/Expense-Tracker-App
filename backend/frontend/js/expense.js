...
const baseUrl = `http://16.171.170.198:3000/api/expense`;
...
document.getElementById("rzp-button1").onclick = async function (e) {
  ...
  const response = await axios.get("http://16.171.170.198:3000/api/purchase", {
    headers: { Authorization: token },
  });
  ...
  var options = {
    ...
    handler: async function (response) {
      const res = await axios.post(
        "http://16.171.170.198:3000/api/purchase/updateTransactionStatus",
        {
...
async function downloadReport() {
  ...
  try {
    ...
    const response = await axios.get(
      "http://16.171.170.198:3000/api/expense/download",
      {
        headers: { Authorization: token },
      }
    );
